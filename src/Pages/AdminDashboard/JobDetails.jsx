...
import { approveJob, getAllCandidatesOfJob, getJobById, getPanelDetails, ListXIPanels, updatePanelIdofJob } from "../../service/api";
    ...
    const getData = async () => {
      ...
      if (res) {
        ...
        if (jobDetails.panelId) {
          console.log('YES')
          let currentPanelDetails = await getPanelDetails({panelId : jobDetails.panelId})
        ...
        await ls.set("jobDetails", JSON.stringify(res.data.job[0]));
            ...
            <div className="card-body md:px-7 md:w-4/5">
              ...
              {job.perks && (
                ...
                <div className="my-7">
                  ...
                  >{xiPanels ? xiPanels.find((x)=>x._id === job.panelId)?.panel : 'NO Panel'}</h6>
                 ...
                 <select onChange={(e)=> setSelectedPanel(e.target.value)}>
                  {xiPanels && xiPanels.length > 0 && xiPanels.map((panel)=>(
                    <option value={panel._id}>{panel.panel}</option>
                  ))}
                 </select>
                 <button className="bg-blue-500 rounded-sm text-white px-2 py-1 cursor-pointer" onClick={()=>updatePanelId()}>Update Panel</button>
                </div>
                ...
                {candidates.length > 0 && (
                  <div className="overflow-x-auto">
                    <table className="w-full my-5 ">
                      ...
                      <tbody>
                        {console.log('CANDIDATES-----------' , candidates ? candidates : 'NO CAND')}
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-left">
                                <a href="/">{user.FirstName ? user.FirstName : user.firstName} {user.LastName ? user.LastName : user.lastName}</a>
                              </td>
                              <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-left">
                        ...
                        {candidates.map((user, index) => {
                          return (
                            ...
                            >
                              ...
                              <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-left">
                                {user.Uid ? <><a href={"/admin/AdminUserProfile/"+user.Uid}>{user.FirstName ? user.FirstName : user.firstName} {user.LastName ? user.LastName : user.lastName}</a></> : <>{user.FirstName ? user.FirstName : user.firstName} {user.LastName ? user.LastName : user.lastName}</> }
                              </td>
                              <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-left">
                                {user.Email ? user.Email : user.email}
                              </td>
                                  ...
                                  {user.Status==="Invited"?
                                    ...
                                    {gcaneval?<>
                                      {gcaneval[index].status === "nf"?
                                        ...
                                        :
                                        (() => {
                                          if(gcaneval[index].interviewState===4){
                                            return (
                                          ...
                                          }else if(gcaneval[index].interviewState===3){
                                            return (
                                          ...
                                          }else if(gcaneval[index].interviewState===2){
                                            return (
                                          ...
                                          }else if(gcaneval[index].interviewState===1){
                                            return (
                                          ...
                                          }else if(gcaneval[index].interviewState===0){
                                            return (
                                              <span className="bg-gray-400 text-gray-800 text-xs font-medium px-3 py-0.5 rounded-3xl dark:bg-yellow-200 dark:text-gray-900 my-2 py-2">
                                                <strong>Invited</strong>
                                              </span>
