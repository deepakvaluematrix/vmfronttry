...
const UpdateJob = () => {
  ...
  // const resdetail1=JSON.parse(getStorage("jobsdetails"))
  // resdetail1.map((item)=>{
  ...
  React.useEffect(() => {
    ...
    const getData = async () => {
      ...
      if (res) {
        ...
        await ls.set("postjob", JSON.stringify(res.data.job))
...
if(data.eligibility){
  ...
  const postJob = async (values) => {
    let access_token = getStorage("access_token");
    let jobs = JSON.parse(await getStorage("postjob"));

    
    const job_id=getStorage("ids");
  ...
  const saveEligible = async (content) => {
    ...
    ls.set("postjob", JSON.stringify(content));
    swal({
      icon: "success",
      title: "Update Job",
      text: "Details Updated Succesfully",
  ...
  //   const job_id=JSON.parse(getStorage("ids"))
  //   values.user_id = job._id;
  //   values.job_id = job_id;
                                ...
                                if (e.key === "Enter" && disabled === false) {
                                  if (inputRef.current) {
                                    if (inputRef.current.value !== "") {
                                      ...
                                      let res = await getStorage("postjob");
                                      res = JSON.parse(res);
                                      res.skills = t;
                                      await ls.set(
                                        "postjob",
