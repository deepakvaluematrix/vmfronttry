...
const BillingDetailForm = (props) => {
  ...
  React.useEffect(() => {
    const initial = async () => {
      let e = JSON.parse(await getStorage("companyDetails"));
  ...
  const handleSubmit = async () => {    
    let res = JSON.parse(await getStorage("companyDetails"));
    ...
    if (response && response.status === 200) {
      await ls.set("user", JSON.stringify(response.data.user));
      // await ls.remove("companyDetails");
          ...
          <div className={`${!showForm ? "hidden" : "block"}`}>
            <Formik
              ...
              onSubmit={async (values) => {
                // //console.log(values);
                let e = JSON.parse(
                  await getStorage("companyDetails")
                );
                if (edit !== null) {
                  ...
                  await ls.set("billing", JSON.stringify(temp));
                  return;
                }
